...
test_indices = []
with open('sonar-scale-test-indices.txt') as f:
    for line in f:
        line = line.rstrip()
...
with open('sonar_scale.txt') as f:
    X_train = []
    Y_train = []
    X_test = []
    Y_test = []
...
for C in C_list:
    ...
    accuracy = 0
    for i in range(5):
        indices_train = list(set(range(0, 150)) ^ set(range(30*i, 30*(i+1))))
        indices_test = list(range(30*i, 30*(i+1)))
        ...
        p_label, p_acc, p_val = predict(val_set_y, val_set_x, m)
